...
void DMA_InitializeHDMA()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        uint8 line = DMARead((dmaAAddrBank[i] << 16) | dmaTablePtr[i]);
        if (line == 0x00)
        {
            hdmaEn[i] = false;
        }
        ...
        if (dmaIndir[i])
        {
            ...
            dmaDataPtr[i] = DMARead(addL) | (DMARead(addH) << 8);
            dmaTablePtr[i]++;
        }
        else
        {
    ...
    if (hBlank)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (hdmaDoTransfer[i])
            {
                ...
                case 0:
                {
                    data = DMARead(dataAddr);
                    DMAWrite(dmaBAddr[i], data);
                    bytesTrf = 1;
                    break;
                }
                case 1:
                {
                    ...
                    data = DMARead(dataAddr);
                    DMAWrite(dmaBAddr[i], data);
                    data = DMARead(dataAddr + 1);
                    DMAWrite(dmaBAddr[i] + 1, data);
                    bytesTrf = 2;
                    break;
                }
                case 6:
                ...
                case 3:
                {
                    ...
                    data = DMARead(dataAddr + 1);
                    DMAWrite(dmaBAddr[i], data);
                    data = DMARead(dataAddr + 2);
                    DMAWrite(dmaBAddr[i] + 1, data);
                    data = DMARead(dataAddr + 3);
                    DMAWrite(dmaBAddr[i] + 1, data);
                    bytesTrf = 4;
                    break;
                }
                case 4:
                {
                    ...
                    DMAWrite(dmaBAddr[i] + 1, data);
                    data = DMARead(dataAddr + 2);
                    DMAWrite(dmaBAddr[i] + 2, data);
                    data = DMARead(dataAddr + 3);
                    DMAWrite(dmaBAddr[i] + 3, data);
                    bytesTrf = 4;
                    break;
                }
                case 5:
            ...
            if (dmaLineCnt[i] == 0) // Re-initialize for the next table
            {
                ...
                line = DMARead((dmaAAddrBank[i] << 16) | dmaTablePtr[i]);
                if (line == 0x00)
                {
                    hdmaEn[i] = false;
                }
